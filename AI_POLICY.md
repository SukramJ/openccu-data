...
openccu-data. It is modeled on the
...
gates: `pytest tests/` and the pre-commit hooks (`prek run --all-files`, which include `ruff`
and `mypy`). AI assistance does not lower the bar.
The committed data artifacts under `openccu_data/data/` must be reproduced with the repository's
generation scripts (`openccu-extract-easymodes`, `openccu-extract-translations`,
`openccu-extract-profiles`, or their `script/extract_*.py` wrappers) — never hand-edited or
AI-fabricated.

## How this project itself uses AI

In the interest of transparency: openccu-data is partly developed with AI assistance
